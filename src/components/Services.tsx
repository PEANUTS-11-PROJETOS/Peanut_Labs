  ...
  if (kind === "landing") {
    return (
      <svg width="48" height="48" viewBox="0 0 48 48" className="text-pl-accent">
        ...
        <path d="M24 31 L26 31 L24 33 Z" fill="var(--color-pl-bg)" />
...
function ServiceModal({
  ...
  const handleKey = useCallback(
    (e: KeyboardEvent) => {
      ...
      // Armadilha de foco: o Tab circula dentro do painel em vez de escapar
      // para o conteúdo atrás do backdrop.
  ...
  useEffect(() => {
    // Guarda quem abriu o modal para devolver o foco no fechamento.
  ...
  return (
    ...
    >
      <div
        ...
        className="modal-in relative w-full sm:max-w-[560px] bg-pl-bg-2 border border-pl-line-2 rounded-t-3xl sm:rounded-2xl outline-none overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* drag handle — mobile only */}
        <div className="sm:hidden flex justify-center pt-3 pb-1">
          <div className="w-10 h-1 rounded-full bg-pl-ink-mute" />
        </div>

        {/* header */}
        <div className="flex items-start justify-between px-6 pt-5 pb-4 border-b border-pl-line">
          <div>
            ...
            <h3 className="font-[family-name:var(--font-space-grotesk)] font-medium text-pl-ink text-2xl leading-tight mt-1">
              {service.title}
            </h3>
          </div>
          <button
            onClick={onClose}
            aria-label="Fechar"
            className="tap-44 relative mt-1 w-8 h-8 rounded-full border border-pl-line-3 flex items-center justify-center text-pl-ink-dim hover:text-pl-ink hover:border-pl-line-hi transition shrink-0"
          >
            ×
          </button>
        </div>
        ...
        <div className="px-6 py-5 space-y-5 max-h-[60vh] overflow-y-auto">
          <div>
            <span className="font-[family-name:var(--font-jetbrains-mono)] text-2xs uppercase tracking-[0.18em] text-pl-ink-dim block mb-1.5">
              O que é?
            </span>
            <p className="text-pl-ink-3 text-base leading-[1.6]">{service.modal.what}</p>
          </div>
          ...
          <div>
            ...
            <p className="text-pl-ink-3 text-base leading-[1.6]">{service.modal.who}</p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {service.modal.receives.map((item, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-pl-ink-2">
                  <span className="font-[family-name:var(--font-jetbrains-mono)] text-2xs text-pl-ink-dim w-5 shrink-0 mt-0.5">
        ...
        {/* footer */}
        <div className="px-6 py-5 border-t border-pl-line flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <div className="font-[family-name:var(--font-space-grotesk)] font-medium text-pl-ink text-xl leading-none">
              {service.modal.price}
            </div>
            <div className="font-[family-name:var(--font-jetbrains-mono)] text-2xs uppercase tracking-[0.15em] text-pl-ink-dim mt-1">
              {service.modal.prazo}
            </div>
          </div>
          <a
            ...
            onClick={onClose}
            className="group inline-flex items-center justify-between gap-3 bg-pl-accent text-pl-bg font-semibold text-sm pl-5 pr-2 py-2.5 rounded-full glow-amber w-full sm:w-auto"
          >
            <span>Solicitar orçamento</span>
            <span className="w-7 h-7 rounded-full bg-pl-bg text-pl-accent flex items-center justify-center transition-transform group-hover:translate-x-0.5">
              →
            </span>
          </a>
        </div>
...
function ServiceCard({
  ...
  return (
    <div
      className="bracket card-tilt relative bg-pl-bg-2/60 backdrop-blur-sm border border-pl-line-2 rounded-2xl p-6 md:p-7 flex flex-col min-h-[480px]"
      data-reveal
    ...
    >
      ...
      <span className="self-start font-[family-name:var(--font-jetbrains-mono)] text-2xs uppercase tracking-[0.18em] text-pl-accent border border-pl-accent/30 bg-pl-accent/8 px-2 py-1 rounded-full mb-5">
        {tag}
      </span>

      <h3 className="font-[family-name:var(--font-space-grotesk)] font-medium text-pl-ink text-2xl md:text-3xl leading-[1.05] tracking-tight mb-3">
        {title}
      </h3>
      <p className="text-pl-ink-dim text-sm leading-[1.6] mb-7">{desc}</p>

      <ul className="mt-auto space-y-2.5 border-t border-pl-line pt-5">
        {bullets.map((b, j) => (
          <li key={b} className="flex items-center gap-3 text-sm text-pl-ink-2">
            <span className="font-[family-name:var(--font-jetbrains-mono)] text-2xs text-pl-ink-dim w-7">
      ...
      <div className="mt-4 pt-4 border-t border-pl-line flex items-end justify-between gap-4">
        <div>
          <span className="font-[family-name:var(--font-jetbrains-mono)] text-2xs uppercase tracking-[0.18em] text-pl-ink-dim block mb-1">
            {priceLabel}
          </span>
          <span className="font-[family-name:var(--font-space-grotesk)] font-medium text-pl-accent text-3xl leading-none">
            {priceValue}
          </span>
        </div>
        <button
          ...
          className="tap-44 relative w-9 h-9 rounded-full border border-pl-line-3 flex items-center justify-center text-pl-ink-3 hover:bg-pl-accent hover:text-pl-bg hover:border-pl-accent transition cursor-pointer shrink-0"
        >
          →
        </button>
      </div>
    ...
    <>
      <section id="servicos" className="relative py-32 overflow-hidden">
        ...
        <div className="relative max-w-[1320px] mx-auto px-5 md:px-10">
          <div className="flex items-end justify-between mb-16 flex-wrap gap-6">
            <div>
              ...
              <h2 className="font-[family-name:var(--font-space-grotesk)] font-medium text-pl-ink text-[clamp(32px,9vw,44px)] md:text-[68px] leading-[0.95] tracking-[-0.025em] max-w-[16ch]">
                Três módulos.<br />
                <span className="text-pl-ink-mute">Um objetivo: </span>
          ...
          <div className="mt-14 flex flex-wrap items-center gap-4 justify-between border-t border-pl-line pt-8">
            <p className="text-pl-ink-dim text-base">
              Não sabe qual módulo faz mais sentido?
              <span className="text-pl-ink-2"> Eu te ajudo a decidir em 1 conversa.</span>
            </p>
            <a
              href="#contato"
              className="inline-flex items-center gap-2 min-h-11 text-sm text-pl-ink border border-pl-line-3 px-4 py-2 rounded-full hover:border-pl-line-hi hover:bg-pl-fill-1 transition"
            >
              <span className="font-[family-name:var(--font-jetbrains-mono)] text-2xs uppercase tracking-[0.18em] text-pl-accent">
                Pedir indicação
              </span>
